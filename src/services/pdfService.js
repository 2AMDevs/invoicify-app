...
/**
 * @async
 * @return Buffer of Selected Font
 */
...
/**
 * @param {string} previewPath Path of BG of PDF
 * @param {boolean} isPreviewMode represents
 * @return pdfDoc generated by pdf-lib
 */
const getPdfDoc = async (previewPath, isPreviewMode) => {
  let pdfDoc
  const isValid = await isValidPath(previewPath)
  if (isPreviewMode) {
    if (!isValid) {
...
/**
 * @param {string} mode Mode of PDF
 * @param {object} page Page object of PDF
 * @param {object} printSettings Get printing settings
 * @param {string} font Base64 buffer of font
 * @param {number} fontSize represents font size
 */
const printDuplicate = async (mode, page, printSettings, font, fontSize) => {
...
/**
 * @param {object} page Page object of PDF
 * @param {object} meta Invoice Header info
 * @param {string} font Base64 buffer of font
 * @param {number} fontSize represents font size
 */
const makeInvoiceHeader = (page, meta, font, fontSize) => {
...
/**
 * @param {object} page Page object of PDF
 * @param {Array} items Detailed list of invoice item
 * @param {object} printSettings Get printing settings
 * @param {string} font Base64 buffer of font
 * @param {number} fontSize represents font size
 */
const printItems = (page, items, printSettings, font, fontSize) => {
...
/**
 * @param {object} page Page object of PDF
 * @param {object} footer Footer details
 * @param {object} printSettings Get printing settings
 * @param {string} font Base64 buffer of font
 * @param {number} fontSize represents font size
 */
const printFooter = (page, footer, printSettings, font, fontSize) => {
  const commonFont = { font, size: fontSize }
 ...
 * @param {object} page Page object of PDF
 * @param {Array} items Detailed list of invoice item
 * @param {object} commonFont represents font size & buffer
 */
const printOldPurchase = (page, items, commonFont) => {
  const oldItems = {}
 ...
 * @param {number} fontSize represents font size
 */
const printPaymentDist = (page, footer, font, fontSize) => {
  const commonFont = { font, size: fontSize }
  const startX = 210
  const yLimit = 190
  let startY = 220
...
/**
 * @param {object} invoiceDetails All details of Invoice in State
 * like, meta, itemDetail, footer, all pay details etc.
 * @param {string} mode Mode of PDF generated
 * @return Base64 of Generated PDF(s)
 */
const getPdf = async (invoiceDetails, mode = PRINT) => {
  ...
  const pdfDoc = await getPdfDoc(previewPath, isPreviewMode)
